...
import { addExercise, deleteExercise, resetAddedExerciseState, resetDeletedExerciseState,
  resetRetrievedExercisesState, resetUpdatedExerciseState, retrieveExercises, updateExercise
...
export class ExercisesPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    console.log("exercises onInit")
  }

  ngOnDestroy() {
    console.log("exercises onDestroy")
  ...
  private onUpdatedExerciseSuccess(loggingExercisesState: LoggingExercisesState) {
  ...
  onSaveClickAdd() {
    this.store.dispatch(addExercise({
      ...
      exerciseType: this.selectedNewExercise.split(';')[2]
    }));
  ...
  onSaveClickEdit() {
    this.store.dispatch(updateExercise({
      ...
      exerciseType: this.editingExercise!.exerciseType!
    }));
  }
  ...
  private updateExerciseIndexOrders(exercises: Exercise[]): void {
    exercises.forEach((exercise, index) => {
      ...
      this.store.dispatch(updateExercise({
        ...
        indexOrder: updatedExercise.indexOrder,
        splitId: this.currentSplitId!,
        sessionId: this.currentSessionId!,
