...
import { addWorkout, resetAddedWorkoutState } from 'src/store/workouts/workouts.actions';
...
export class WorkoutPage implements OnInit, OnDestroy {
  ...
  startNewRestIntervalTime() {
    ...
    const currentExerciseType = this.groupedExercises[this.groupedExerciseIndex][0].exerciseType!.trim();
  
    let isolationRestInterval = 120;
    let compoundRestInterval = 180;
  
    if (this.totalSeconds / 60 > 50) {
      const additionalRestTime = Math.floor((this.totalSeconds / 60 - 50) / 10) * 15;
  
      isolationRestInterval += additionalRestTime;
      compoundRestInterval += additionalRestTime;
    }
  
    this.restIntervalTimer = (currentExerciseType == 'isolation') ? isolationRestInterval : compoundRestInterval;
  ...
  finishWorkout() {
    const data: { exerciseId: string; exerciseName: string, weight: any; reps: any; exerciseType: string }[] = [];
